...
def update_info(user, agent, use_another_task):
    ...
    filters="agent=%s" % agent.name
    response, content = callRestServer(user, filters, "agentinfo", "desc", "agentname="+agent.name, wait_response=True, use_task=use_another_task, limit=1)
    ...
    if response.getStatus() == 200:
        for msg in content:
            ...
            logger.warn("No server to call for agent %s" % agent.name)
