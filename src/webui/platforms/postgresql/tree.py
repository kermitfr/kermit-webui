...
class PosgreSQLServerTree(ServerTree):
    ...
    def getDetailsTree(self, hostname):
        ...
        if server_info:
            content = {"isFolder": True, "expand": True, "title": 'PostgreSQL', "key":'PostgreSQL', "icon":"postgresql.png", "detailsEnabled":"true", 'url': reverse('jboss_details', kwargs={'hostname':hostname, 'instance_name':hostname, 'resource_name':hostname})}
            logger.debug('Configuring Databases')
            db_instances = {'title': 'Databases', 'isFolder':True, "expand":True, "key":"databases", "icon":"folder_database.png", "type":"instances"}
            dbs = []
            for database in server_info["databases"]:
                db = {'title':database['name'], "key":database['name'], "icon":"database.png", "type":"database", "instance":database['name']}
                ...
                for table in database['tables']:
                    tab = {'title':table, "key":table, "icon":"datasource.png", "type":"application", "database":database['name']}
                    tabs.append(tab)
