    ...
    if request.method == "POST":
        ...
        if xhr == "xhr":
            ...
            if sql_script:
                ...
                #;user=%s;userip=%s
                response, content = callRestServer(request.user, filters, 'oracledb', 'execute_sql', 'sqlfile=%s;dbname=%s' % (sql_script, dbname))
    ...
    if request.method == "POST":
        ...
        if xhr == "xhr":
            ...
            if instance and schema and target_server and target_instance:
                ...
                response, content = callRestServer(request.user, filters, 'oracledb', 'export_database', 'instancename=%s;schema=%s' %(instance, schema))
                if response.status == 200:
                    json_content = json.loads(content)
